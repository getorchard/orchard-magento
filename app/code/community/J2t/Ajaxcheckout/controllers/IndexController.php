...
class J2t_Ajaxcheckout_IndexController extends /*Mage_Checkout_CartController*/ Mage_Core_Controller_Front_Action
{
        ...
        try {
            if (isset($params['qty_super_attribute'])){
                ...
                if (!$hasbeenprocessed){
                    ...
                    $message_type = Mage_Core_Model_Message::ERROR;
                    $this->_renderConfirmationBox($message, $message_type);
                    return;
                }
                $cart->save();
            } else {
            ...
            $message_type = Mage_Core_Model_Message::SUCCESS;
            ...
            if ($this->_getSession()->getUseNotice(true)) {
                ...
                $message_type = Mage_Core_Model_Message::ERROR;
                $this->_renderConfirmationBox($message, $message_type, $img);
                return;
            } else {
    ...
    {   
            ...
            if ($this->getRequest()->getParam('cart') == "delete"){
                ...
                if ($id) {
                    try {
                        ...
                        //Mage::getSingleton('checkout/session')->addError($this->__('Cannot remove item'));
                        $message = $this->__('Cannot remove item.');
                        $message_type = Mage_Core_Model_Message::ERROR;
                        $this->_renderConfirmationBox($message, $message_type);
                        return;
                    }
            ...
            if ($productId) {
                ...
                try {
                    ...
                    if (Mage::getSingleton('checkout/session')->getUseNotice(true)) {
                        ...
                        $message = $e->getMessage();
                        $message_type = Mage_Core_Model_Message::NOTICE;
                        $this->_renderConfirmationBox($message, $message_type);
                        return;
                    } else {
                        $messages = array_unique(explode("\n", $e->getMessage()));
        ...
        $this->_initLayoutMessages('checkout/session');
        $block = $this->getLayout()->getBlock('j2tajaxcheckout'); 
        $block->setData('j2t_message', $message);
        $this->renderLayout();*/
        $this->_renderConfirmationBox($message, $message_type, $img);
        return;
    }
    
    protected function _renderConfirmationBox($message = '', $message_type = Mage_Core_Model_Message::SUCCESS, $img = null){
        $layout = $this->loadLayout();
        $this->_initLayoutMessages('checkout/session');
        $block = $this->getLayout()->getBlock('j2tajaxcheckout'); 
        $block->setData('j2t_message', $message);
        $block->setData('j2t_message_type', $message_type);
        $block->setData('j2t_img', $img);
        $this->renderLayout();
        //exit;
    }
    ...
    public function productcheckAction()
    {
        ...
        if($product_id = $params['product']){
            ...
            if ($product->getId()){
                ...
                if (in_array($product->getId(), $array_restrictions)){
                    $url = $product->getProductUrl();
                    echo $url."J2T_URL_REDIRECT";
                    die;
                }
