<?php
/**
 * J2T-DESIGN.
 *
 * @category   J2t
 * @package    J2t_Ajaxcheckout
 * @copyright  Copyright (c) 2003-2009 J2T DESIGN. (http://www.j2t-design.com)
 * @license    OSL
 */

class J2t_Ajaxcheckout_Block_J2thead extends Mage_Core_Block_Template
{
    protected function _prepareLayout()
    {
        parent::_prepareLayout();
        
        $active = Mage::getStoreConfig('j2tajaxcheckout/default/j2t_ajax_cart_active', Mage::app()->getStore()->getId());
        if ($active){
            $model_name = Mage::getStoreConfig('j2tajaxcheckout/default/j2t_ajax_cart_template', Mage::app()->getStore()->getId());
            $min = Mage::getStoreConfig('j2tajaxcheckout/default/j2t_ajax_cart_minified', Mage::app()->getStore()->getId());

            $js = ($min) ? "min-" : "";

            $this->getLayout()->getBlock('head')->addItem('skin_js','js/j2t/'.$js."ajax_cart.js");
            $this->getLayout()->getBlock('head')->addItem('skin_css','css/j2t/'.$js."ajax_cart.css");

            if ($model_name != 'default'){
                $temp = Mage::getModel("j2tajaxcheckout/template$model_name");
                $this->getLayout()->getBlock('head')->addItem('skin_css','css/j2t/'.$js.$temp->getCssName());
            }
        }
        
    }

}
